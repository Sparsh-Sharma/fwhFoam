...
The same simulation, sampled on a permeable cylindrical control surface,
yields the source-localization density~\eqref{eq:sigma} at the shedding
tone (Fig.~\ref{fig:sigma}). The density concentrates on the flanks of the
surface normal to the mean flow, consistent with the lift-dipole mechanism,
and its surface integral reproduces the independently computed far-field
power to within a few percent --- the built-in identity holding on genuine
CFD data.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{figs/sigma_cylinder.pdf}
\caption{Source-power density $\sigma$ on the permeable control surface at
the shedding tone: (left) azimuthal profile, peaking at $\theta=\pm90^\circ$
(normal to the flow); (right) $\sigma$ over the control surface. The signed
density localizes the radiating regions; $\oint\sigma\,dS$ matches the
far-field power.}
\label{fig:sigma}
\end{figure}
